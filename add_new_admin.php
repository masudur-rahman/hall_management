...
<?php
    ob_start();
    session_start();
    if (!isset($_SESSION['username']) or $_SESSION['idtype']!='admin') {
...
<body>
    <?php
        ...
        if($_SERVER["REQUEST_METHOD"]=="POST"){
			...
			$cnt1=mysqli_num_rows($rslt);
           	...
           	if($cnt>0 or $cnt1>0 or $_POST['pwd']!=$_POST['rpwd'] or strlen($_POST['pwd'])<5)
           	{
            ...
            if(!$conn->connect_error){
              $sql = "INSERT INTO admin_info(Username,Password, Fname, Lname, Email, Mobile) 
              VALUES ('".$_POST["userid"]."','".$_POST["pwd"]."','".$_POST["fname"]."','".$_POST["lname"]."','".$_POST["email"]."','".$_POST["phn"]."')";
            }
            if($conn->query($sql)){
              $_SESSION['info']= "<script type='text/javascript'>$.notify('New Admin Added Successfully..','success')</script>";
    ...
    <center>
        <div class="w3-container w3-row " style="background-image: url(images/sha12.jpg">

            <div class="w3-col" style="width: 10%">
                <br>
            </div>
            <div class="w3-col w3-whitee" style="width: 80%;background: #3ee3a6; color: black; background-image: url(images/back2.jpg)">
                <h2 style="font-family:Imprint MT Shadow; color: brown;"><b>Add New Admin</b></h2>
                <div class="w3-row">
                ...
                <div class="form-group">
                    <button style="margin-left: -35px" type="submit" name="submit" class="btn btn-success">Add Admin</button>
                </div>
                </form>
            </div>
