...
<body>
    ...
    <?php 
        if(isset($_POST['approve'])){
            $id=$_POST['approve'];
            $sql="UPDATE token SET Approved='Yes' WHERE ID=$id";
    ...
    <?php include('header.php'); ?>
        ...
        <div class="w3-col w3-container w3-teall" style="width: 100%; background: rgb(12, 141, 154)">
            <center><h2 style="font-family: Imprint MT shadow; color: cyan">Token Entry List</h2></center> <hr>
            <div id="dd" style="background : khaki">
                <table id="t01">
                    ...
                    <?php
                        $sql="SELECT * FROM token WHERE Approved='No'";
                        $rslt=mysqli_query($conn, $sql);

                        if(mysqli_num_rows($rslt)){
                            while($row=mysqli_fetch_assoc($rslt)){
                                $id=$row['ID']; $name=$row['Name']; $stid=$row['StID']; $room=$row['Room_No']; $token=$row['Token']; ?>
                                <tr><td id='t03'><?php echo $name; ?></td><td id='t03'><?php echo $stid; ?></td>
                                <td id='t03'><?php echo $room; ?></td><td id='t03'><a href='<?php echo $token;?>' target='_blank' style='font-size:115%'>View</a></td><form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method='POST'><td id='t03'><button id='c' name='approve' value='<?php echo $id; ?>'>&#10004</button><button name='deny' id='d' value='<?php echo $id; ?>'>&#10008</button></td></form>
